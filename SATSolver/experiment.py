...
def main():
    ...
    for file in os.listdir(aux_folder):
        i_time = time.time()
        thruth, solution, n_splits, n_backtracks = SAT(file,0)
        f_time = time.time()
        ...
        save_result_as_DIMACS(solution, file, n_splits, n_backtracks, Total_time)
...
def save_result_as_DIMACS(solution, filename, n_splits, n_backtracks, Total_time):
    with open(f'{solutions_folder}/{filename}','w+') as f:
...
def prepare_folders():
    ...
    if os.path.exists(solutions_folder):
        for file in os.scandir(solutions_folder):
            os.remove(file.path)
